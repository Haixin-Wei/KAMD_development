...
! derived from them that are needed during mdin &ctrl printout, etc.
  ...
  integer                       imin, nmropt, ntx, irest, ntrx, ntxo, &
                                ...
                                baroscalingdir, & ! box scaling direction
                                icfe, ifsc, iskip_ff, sceeorder, logdvdl, ifmbar, ifmbar_net, &
                                bar_intervall, klambda, barostat, mcbarint, &
                                scaledMD, icnstph, ntcnstph, ntrelax, icnste, &
                                ntcnste, ntrelaxe, ntwf, &
                                iphmd, & !PHMD
                                igamd,irest_gamd,iE,iEP,iED,iEB,iVm,igamdlag,ntcmdprep,ntebprep, &
                                ntcmd,nteb,ibblig,nlig,atom_p,atom_l,ntmsd,nftau, &
                                bgpro2atm,edpro2atm,&
                                plumed, & ! PLUMED
                                w_amd, emil_do_calc,isgld,isgsta,isgend,sgtype, &
                                tishake, emil_sc,iemap, lj1264, plj1264, efn,mcwat,mcint,&
                                ...
                                gti_cut, gti_cut_sc, gti_add_sc, gti_bat_sc, gti_lam_sch, & ! GTI
                                gti_ele_gauss, gti_ele_sc, gti_vdw_sc, gti_chg_keep, & ! GTI
                                gti_auto_alpha, gti_scale_beta, gti_ele_exp, gti_vdw_exp, & !GTI
                                gti_heating, gti_hsteps1,gti_hsteps2, & ! GTI
                                gti_resv_accept, gti_resv_fit, & ! GTI
                                ...
                                ifreaf, gti_tau_sch, gti_add_re, & ! REAF
                                rmsd_type, &
                                reweight, iextpot, mcwatretry, &
                                ...
                                asm, & !ASM
                                ntwd,& ! decomp
                                refer_1_start, refer_1_end, refer_2_start, &
                                refer_2_end, refer_3_start, refer_3_end, &
                                refer_4_start, refer_4_end, refer_5_start, &
                                refer_5_end, refer_6_start, refer_6_end, &
                                refer_2_equal, refer_3_equal, refer_4_equal, &
                                refer_5_equal, refer_6_equal, &
                                solvent_start, solvent_end, pseudoBD_cycles ! Haixin
  ...
  common / mdin_ctrl_int /      &
             imin, nmropt, ntx, irest, ntrx, ntxo, &     !  6
             ...
             ihwtnm, iowtnm, iwtnm, ips, csurften, &     ! 52
             ninterface, iamd, iamdlag, timlim, &        ! 56
             icfe, ifsc, iskip_ff, sceeorder, logdvdl, ifmbar, &   ! 62
             bar_intervall, klambda, barostat, &         ! 65
             mcbarint, scaledMD, icnstph, ntcnstph, &    ! 69
             ntrelax,ntwf,w_amd, emil_do_calc, &         ! 73
             isgld,isgsta,isgend,sgtype, tishake,     &  ! 78
             emil_sc, iemap, lj1264, plj1264, &          ! 82 New2022
             igamd,irest_gamd,iE,iVm,igamdlag,ntcmd,nteb, & ! 89
             ntcmdprep,ntebprep,bgpro2atm,edpro2atm,efn,infe,irandom, &  ! 96
             icnste,ntcnste,ntrelaxe,             &  ! 99
             mask_from_ref, mbar_states,           &   ! 101
             gti_eq_nstep, gti_init_vel,  gti_syn_mass, & !104
             gremd_acyc, gti_cpu_output, gti_output, & !107
             gti_cut, gti_cut_sc, gti_add_sc, gti_bat_sc, gti_lam_sch, & !112
             gti_ele_sc, gti_vdw_sc, gti_chg_keep, & !115
             gti_heating, & !116
             gti_auto_alpha, gti_scale_beta,  & !118
             gti_ele_exp, gti_vdw_exp, & !120
             gti_hsteps1,gti_hsteps2, & !122
             gti_resv_accept, gti_resv_fit, &!124
             gti_resv_anch, & !125 
             mcint,nmd,mcrescyc,nmc,mcverbosity,& !130
             ineb,skmin,skmax,tmode,vv,  & !135
             iphmd, midpoint, baroscalingdir, & !138
             reservoir_exchange_step, & !139
             ifsams, sams_type, sams_scan, & !142
             ifreaf, gti_tau_sch, gti_add_re, & !145
             sams_restart, sams_verbose, sams_variance, & !148
             sams_lambda_start, sams_lambda_end, & !150
             sams_init_start, sams_stat_start, & !152
             sams_opt_end, sams_jump_range, sams_update_freq, &!155
             rmsd_type, & !156
             ifmbar_net, nebfreq , ramdint, ramdboostfreq, &  !160
             plumed, & !161 PLUMED
             reweight, iextpot, & !163
             ionstepvelocities,iEP,iED,ibblig,nlig,atom_p,atom_l, & !170
             ntmsd,nftau, & !172
             mcwatretry, & !173
             gbion, &       !174
             ...
             refer_2_equal, refer_3_equal, refer_4_equal, &
             refer_5_equal, refer_6_equal, &
             solvent_start, solvent_end, pseudoBD_cycles ! Haixin 199

  ! This variable must be set to the total number of integers in the
  ! mdin_ctrl_int common block.
  integer, parameter    :: mdin_ctrl_int_cnt = 207 !! add 8 for the two extra arrays
  save  :: / mdin_ctrl_int /
  ...
  double precision              dielc, es_cutoff, vdw_cutoff, fswitch, &
                                ...
                                screen_s, screen_p, gamma_ten, EthreshD, EthreshB,&
                                ...
                                mcboxshift, mccoarsethresh, mcligshift, &
                                sams_alpha, sams_beta, sams_gamma, &
                                gti_vdw_cap, gti_tempi,gti_cut_sc_on, gti_cut_sc_off, &
                                reaf_tau, reaf_temp, &
                                ramdboost, ramdmaxdist, ramdboostrate, &
                                gi_coef, gi_coef_1, gi_coef_2, gb_neckscale_ion_1, gb_neckscale_ion_2, &
                                intdiel_ion_1, intdiel_ion_2, &
                                gi_coef_1_p, gi_coef_1_n, gi_coef_2_pp, gi_coef_2_pn, gi_coef_2_nn, &
                                gb_neckscale_ion_1_p, gb_neckscale_ion_1_n, &
                                gb_neckscale_ion_2_pp, gb_neckscale_ion_2_pn, gb_neckscale_ion_2_nn, &
                                intdiel_ion_1_p, intdiel_ion_1_n, & 
                                intdiel_ion_2_pp, intdiel_ion_2_pn, intdiel_ion_2_nn, &
                                therm_par, & !middle-scheme
                                vel_scale_t

  double precision, dimension(20001) :: mbar_lambda
  double precision, dimension(5)::  rmsd_strength
  ...
  common / mdin_ctrl_dbl /      dielc, es_cutoff, vdw_cutoff, &                  !3
                                ...
                                EthreshD,EthreshB, alphaD, EthreshP, alphaP, &            !62
                                ...
                                tsgavg,sgft,sgff,sgfg,tempsg,tsgavp,sgsize,  &   !83
                                gammamap,                                    &   !84
                                ...
                                sigma0B,sigmaVB,cutoffB,VmaxB,VminB,VavgB,k0B,k_B,& ! 129
                                efx,efy,efz,efphase,effreq,solve,mbar_lambda,mccoarsethresh,& !137
                                vfac,  mcboxshift, mcligshift, & !140
                                sams_alpha, sams_gamma, sams_beta, &  !143
                                gti_tempi, gti_vdw_cap, gti_cut_sc_on, gti_cut_sc_off,&  !147
                                reaf_tau, reaf_temp, & !149
                                ramdboost, ramdmaxdist, ramdboostrate, & ! 152
                                gi_coef, gi_coef_1, gi_coef_2,  gb_neckscale_ion_1, gb_neckscale_ion_2, &  ! 157
                                intdiel_ion_1, intdiel_ion_2, & !159
                                gi_coef_1_p, gi_coef_1_n, gi_coef_2_pp, gi_coef_2_pn, gi_coef_2_nn, & !164
                                gb_neckscale_ion_1_p, gb_neckscale_ion_1_n, & !166
                                gb_neckscale_ion_2_pp, gb_neckscale_ion_2_pn, gb_neckscale_ion_2_nn, & !169
                                intdiel_ion_1_p, intdiel_ion_1_n, & !171
                                intdiel_ion_2_pp, intdiel_ion_2_pn, intdiel_ion_2_nn, & !174
                                therm_par, & !175
                                vel_scale_t  !176

  ! This variable must be set to the total number of doubles in the
  ! mdin_ctrl_dbl common block.
  ! Currently this is 175 (176 minus the mbar_lambda array) + the
  ! size of the mbar_lambda array (20001).
  integer, parameter    :: mdin_ctrl_dbl_cnt = 20177
...
!DG: removed the private type from tgtfitmask and tgtrmsmask
  ...
  namelist /cntrl/      imin, nmropt, ntx, irest, ntrx, ntxo, &
                        ...
                        EthreshD_w, alphaD_w, EthreshP_w, alphaP_w, emil_sc, lj1264, plj1264, & ! New2022
                        igamd,irest_gamd,iE,iED,iEP,iEB,iVm,igamdlag,kD,kP,ntcmd,nteb,ntcmdprep,ntebprep, &
                        VmaxD,VminD,VavgD,k0D,VmaxP,VminP,VavgP,k0P, &
                        sigma0D,sigma0P,sigmaVD,sigmaVP,cutoffD,cutoffP,sigma0B,sigmaVB,cutoffB,VmaxB,VminB,k0B,k_B,fswitch, &
                        ibblig,nlig,atom_p,atom_l,dblig,ntmsd,nftau, &
                        bgpro2atm,edpro2atm, &
                        ...
                        gti_cut_sc,gti_cut_sc_on, gti_cut_sc_off, &
                        gti_add_sc, gti_bat_sc, gti_lam_sch, &
                        gti_auto_alpha, gti_scale_beta, &
                        gti_ele_exp, gti_vdw_exp, &
                        gti_ele_gauss, gti_ele_sc, gti_vdw_sc, gti_chg_keep, gti_vdw_cap, &
                        gti_heating, &
                        gti_hsteps1, gti_hsteps2, gti_tempi, &
                        gti_resv_accept, gti_resv_fit, gti_resv_anch, &                        
                        ifreaf, reaf_tau, reaf_temp, gti_add_re, &
                        mcwat,mcint,nmd,mcrescyc,nmc, mcverbosity,&
                        mcresstr,ineb,skmin,skmax,tmode,vv,vfac,&
                        nebfreq, mccoarsethresh, mcwatretry, mcwatmask, &
                        ...
                        intdiel_ion_1_p, intdiel_ion_1_n, & 
                        intdiel_ion_2_pp, intdiel_ion_2_pn, intdiel_ion_2_nn, &
                        ischeme, ithermostat, therm_par, & !middle_scheme
                        rmsd_strength, rmsd_mask, rmsd_ti, rmsd_type, &
                        gti_resv_velmode, gti_resv_mapmode, &
                        vel_scale_t, asm, &
                        refer_1_start, refer_1_end, refer_2_start, &
                        refer_2_end, refer_3_start, refer_3_end, &
                        refer_4_start, refer_4_end, refer_5_start, &
...
! gbion options:
  gbion = 0    ! default=0, no change
  gi_coef = 0.7071067812d0     ! default=sqrt(0.5), empirical
  gi_coef_1 = 1.0    ! ion-solute coef
  gi_coef_2 = 1.0    ! ion-ion coef
  gb_neckscale_ion_1 = 1.0   ! ion-solute neckscale coef
  gb_neckscale_ion_2 = 1.0   ! ion-ion neckscale coef
  intdiel_ion_1 = 1.0  ! intdiel for ion-solute
  intdiel_ion_2 = 1.0  ! intdiel for ion-ion
  gi_coef_1_p = 1.0
  gi_coef_1_n = 1.0
  gi_coef_2_pp = 1.0
  gi_coef_2_pn = 1.0
...
! PHMD
  ...
  ! Self-guided Langevin dynamics
   ...
   isgld = 0   ! 0--no self-guiding,1--sgldg with uniform guiding forces, recommanded
               !                     2--sgldg with atomic specific guiding forces
               !                     3--sgldg with balanced guiding forces
   isgsta=1    ! Beginning index of SGLD range
   isgend=0    ! Ending index of SGLD range
   sgmask=':*'   ! atom mask to select atoms to apply guiding forces
   tsgavg=0.2d0    !  Local averaging time of SGLD simulation
   sgft=0.0d0     !  Momentum guiding factor of SGLDg simulation
   sgff=0.0d0     !  Force guiding factor of SGLDg simulation
   sgfg=0.0d0      !  Guiding factor of SGLD-GLE simulation
   ...
   sgtype=1       !   1--self atom only, 
                   ...
                   !   4--spatial average with cutoff sgsize
  sgsize=3.0d0        ! local spatial average cutoff distance
...
! GTI addon
  ...
  gti_cut = 1
  gti_cut_sc = 0
  gti_cut_sc_on = fswitch
  gti_cut_sc_off = cut
  gti_add_sc = 1 ! <=0: none 1: 14-elec terms; 2: 14+internal elec
  gti_ele_gauss =0
  gti_auto_alpha=0
  gti_scale_beta=0
  gti_ele_exp=2
...
! Read input in namelist format:
  ...
  if (igb .eq. 0) then
   ...
   ! igb=6 is gas phase via the GB code.
      ...
      !gb_beta_p    = 0.8d0
      !gb_gamma_p   = 4.85d0
      gb_alpha_p   = 0.418365d0 !!Use gbneck2nu parms for P
...
#else
    write (mdout, '(a)') &
      "| ERROR: F95 RANDOM_NUMBER intrinsic selected for random number generator"
    write (mdout, '(a)') &
...
! Check on bogus data and unsupported options:
  ...
  if(mcwat .eq. 1) then
    ...
    if(remd_method .eq. 0) then
      write(mdout, '(a,a)') error_hdr, 'mcwat does not currently support non-REMD MPI!'
      inerr = 1
    end if
#endif
    if(mcint .gt. 0) then
    ...
    if(nmd .lt. 1) then
      write(mdout, '(a,a)') error_hdr, 'nmd must be greater than 0!'
      inerr = 1
    end if
    if(igb .ne. 0) then
    ...
    if(nmc .lt. 1) then
      write(mdout, '(a,a)') error_hdr, 'nmc must be greater than 0!'
      inerr = 1
    end if
    if(mcwatretry .le. 0) then
      write(mdout, '(a,a)') error_hdr, 'mcwatretry must be greater than 0!'
      inerr = 1
    end if
    if(mcboxshift .gt. 0.d0) then
      write(mdout, '(3a)') error_hdr, 'mcboxshift is deprecated; ', &
               'try mcwatmask and mcligshift'
      inerr = 1
    end if
    if(mcligshift .gt. 0.0d0) then
        if(mcwatmask .eq. '') then
          write(mdout, '(a,a)') error_hdr, 'mcwatmask must be set if mcligshift is on'
          inerr = 1
        end if
    end if
  end if
  ...
  if (fswitch .gt. 0) then
    if ((lj1264 .gt. 0) .or. (plj1264 .gt. 0)) then ! New2022
  ...
  ! For Amber versions much older than 20, this was the sole condition:
  ! if (ntx .lt. 1 .or. ntx .gt. 7 .or. ntx .eq. 3) then
  !  write(mdout, '(a,a)') error_hdr, 'ntx must be set to 1, 2, 4, 5, 6, or 7!'
  ! But as of Amber 21, also warn against 2, 4, 6, and 7:
  ! 
  if (ntx .lt. 1 .or. ntx .gt. 7 .or. ntx .eq. 3) then
    write(mdout, '(a,a)') error_hdr, 'ntx must be set to 1 or 5!'
    inerr = 1
  else if (ntx .eq. 2) then
    write(mdout, '(a,a)') warn_hdr, &
    'ntx set to 2; the file type is now auto-detected; please update to ntx = 1.'
  else if (ntx .eq. 4) then
    write(mdout, '(a,a)') warn_hdr, &
    'ntx set to 4; the file type is now auto-detected; please update to ntx = 5.'
  else if (ntx .eq. 6) then
    write(mdout, '(a,a)') warn_hdr, &
    'ntx set to 6; the file type is now auto-detected; please update to ntx = 5.'
  else if (ntx .eq. 7) then
    write(mdout, '(a,a)') warn_hdr, &
    'ntx set to 7; the file type is now auto-detected; please update to ntx = 5.'
  end if
  ...
  if (igamd.gt.0) then
    if (igamd.eq.4 .or. igamd.eq.5) then
      if (ntp.le.0 .or. barostat.ne.1) then
        write(mdout, '(a,a)') error_hdr, &
          'set ntp > 0 and barostat = 1 with PME for running non-bonded GaMD with igamd=4 or 5!'
        inerr = 1
      end if
    end if
    !if (igamd.eq.18 .or. igamd.eq.20 .or.igamd.eq.21.or.igamd.eq.22.or.igamd.eq.23.or.&
    !                igamd.eq.24.or.igamd.eq.25.or.igamd.eq.26.or.igamd.eq.27.or.igamd.eq.28) then
    !  if (ntp.le.0 .or. barostat.ne.1) then
    !    write(mdout, '(a,a)') error_hdr, &
    !      'set ntp > 0 and barostat = 1 for running GaMD with igamd=18, 20-28!'
    !    inerr = 1
    !  end if
    !end if
    ...
    if (sigmaVB .lt. 0) then
       write(mdout, '(a,a)') error_hdr, 'sigmaVB must be >= 0!'
       inerr = 1
    end if
    ...
    if (cutoffB .lt. 0) then
       write(mdout, '(a,a)') error_hdr, 'cutoffB must be >= 0!'
       inerr = 1
    end if
    if (iED .eq. 0) then
    ...
    if (iEB .eq. 0) then
      !write(mdout, '(a,a)') 'set default iEB = iE'
      iEB = iE
    end if
    if (iEP .eq. 0) then
    ...
    if (sigma0B .lt. 0) then
       write(mdout, '(a,a)') error_hdr, 'sigma0B must be >= 0!'
       inerr = 1
    end if
    if(igamd.ge.6.and.igamd.le.11) then
      if (gti_add_sc.ne.1) then
        gti_add_sc=1
        write(mdout, '(a,a)') 'Set gti_add_sc=1 (default) for running LiGaMD with igamd=6 - 11!'
      end if
      if (gti_cpu_output.ne.1) then
        gti_cpu_output=1
        write(mdout, '(a,a)') 'Set gti_cpu_output=1 (default) for running LiGaMD with igamd=6 - 11!'
      end if
    end if
    ...
    if (irest_gamd.eq.1) then
      ...
      if (fexist) then
        ...
        if (igamd.eq.1 .or. igamd.eq.3 .or. igamd.eq.4 .or. igamd.eq.5 .or. igamd.eq.6 .or. &
            ...
            igamd.eq.120 .or. igamd.eq.18 .or. igamd.eq.19 .or. igamd.eq.20.or.igamd.eq.21.or. &
            igamd.eq.22 .or. igamd.eq.23.or.igamd.eq.24.or.igamd.eq.25.or.igamd.eq.26.or.igamd.eq.27.or.&
            igamd.eq.28) then
        ...
        if (igamd.eq.2 .or. igamd.eq.3 .or. igamd.eq.5 .or. igamd.eq.7 .or. &
            ...
            igamd.eq.18.or. igamd.eq.19.or. igamd.eq.20.or.igamd.eq.21.or.igamd.eq.100.or.&
            igamd.eq.22.or.igamd.eq.23.or.igamd.eq.24.or.igamd.eq.25.or.igamd.eq.26.or.igamd.eq.27.or.&
            igamd.eq.28) then
        ...
        if (igamd.ge.20.and.igamd.le.28)then
            read(gamdres,*) VmaxB, VminB, VavgB, sigmaVB
            write(mdout,'(a,4f14.4)')'| Read GaMD restart parameters:VmaxB,VminB,VavgB,sigmaVB = ', &
                                      VmaxB,VminB,VavgB,sigmaVB
        endif
        close(gamdres)
      end if
    end if
...
! gbion valid options
  if (gbion > 0) then
    if (gbion < 0 .or. gbion > 3) then
      write(mdout,'(/2x,a)') &
      'Invalid gbion value. gbion must be 0, 1, 2, or 3.'
      inerr = 1
    end if
    if (ntb /= 0) then
      write(mdout,'(/2x,a)') &
      'ntb invalid. ntb must be 0 for GB ion option.'
      inerr = 1
    end if
    if (igb == 0) then
       write(mdout,'(/2x,a)') &
       'igb invalid. ntp must be >0 for GB ion option.'
       inerr = 1
    end if
    ! gi_coef valid options
    if (gi_coef <= 0.d0) then
      write(mdout,'(/2x,a)') &
      'Invalid gi_coef value. gi_coef must be larger than zero'
      inerr = 1
    end if 
    if (gi_coef_1 <= 0.d0) then
      write(mdout,'(/2x,a)') &
      'Invalid gi_coef_1 value. gi_coef_1 must be larger than zero'
      inerr = 1
    end if
    if (gi_coef_2 <= 0.d0) then
      write(mdout,'(/2x,a)') &
      'Invalid gi_coef_2 value. gi_coef_2 must be larger than zero'
      inerr = 1
    end if
  end if
...
!DG: changed this to allow the use of tgtrmsmask and tgtfitmask in  neb
  ...
  if (ntp .gt. 0 .and. icnstph .ne. 0) then
    write(mdout, '(2a)') error_hdr, 'Currently, constant pH simulations cannot be executed at constant pressure!'
    inerr = 1
  end if

  if (ntp .gt. 0 .and. icnste .ne. 0) then
    write(mdout, '(2a)') error_hdr, 'Currently, constant redox potential simulations cannot be executed at constant pressure!'
    inerr = 1
  end if

  if (iextpot .eq. 1 .and. extprog .eq. '') then
    write(mdout, '(2a)') error_hdr, 'if using external potential code, extprog flag must be set inside the &extpot namelist in the mdin file!'
    inerr = 1
  end if

#ifndef MBX
  if (iextpot .eq. 1 .and. upcase(extprog) .eq. 'MBX') then
    write(mdout, '(2a)') error_hdr, 'pmemd needs to be compiled with the MBX library!'
    inerr = 1
  end if
#endif

  if (iextpot .gt. 0 .and. remd_method .ne. 0 .and. remd_method .ne. 1) then
    write(mdout, '(2a)') error_hdr, 'External library can only be used with no REMD or T-REMD (-rem 1)!'
    inerr = 1
  end if
  ...
  if (iextpot .gt. 0 .and. icfe .gt. 0) then
    write(mdout, '(2a)') warn_hdr, 'TI with external library must calculate external part of generalised force externally, only use if you understand.'
  end if
  ...
  if (ntp .lt. 0 .or. ntp .gt. 4) then
    write(mdout, '(a,a)') error_hdr, 'ntp must be set to 0, 1, 2, 3, or 4!'
    inerr = 1
  end if
...
!igamd=0 no boost is used, 1 boost on the total energy, 2 boost on the dohedrals, 3 boost on dihedrals and total energy
  ...
  if(igamd.gt.0)then
    ...
    if (igamd.eq.1 .or. igamd.eq.3 .or. igamd.eq.4 .or. igamd.eq.5 .or. igamd.eq.6 .or. &
        ...
        igamd.eq.118 .or. igamd.eq.119 .or. igamd.eq.120 .or.igamd.eq.20.or.igamd.eq.21.or.&
        igamd.eq.22.or.igamd.eq.23.or.igamd.eq.24.or.igamd.eq.25.or.igamd.eq.26.or.igamd.eq.27.or.&
        igamd.eq.28) then
      if (iVm.eq.2) then
    ...
    if (igamd.eq.2 .or. igamd.eq.3 .or. igamd.eq.4 .or. igamd.eq.5 .or. igamd.eq.6 .or. &
        igamd.eq.7 .or. igamd.eq.8 .or. igamd.eq.9 .or. igamd.eq.10 .or. igamd.eq.11 .or. &
        igamd.eq.100 .or. igamd.eq.101 .or. igamd.eq.102 .or. igamd.eq.103 .or. igamd.eq.104 .or. &
        igamd.eq.13 .or. igamd.eq.15 .or. igamd.eq.17.or.igamd.eq.18.or.igamd.eq.19 .or.igamd.eq.20.or.&
        igamd.eq.21.or.igamd.eq.22.or.igamd.eq.23.or.igamd.eq.24.or.igamd.eq.25.or.igamd.eq.26.or.igamd.eq.27.or.&
        igamd.eq.28) then
      if (iVm.eq.2) then
    ...
    if(igamd.ge.20.and.igamd.le.28)then
       ...
       if (iVm.eq.2) then
         VmaxB = VavgB + cutoffB * sigmaVB
         VminB = VavgB - cutoffB * sigmaVB
       else if (iVm.eq.3) then
         VmaxB = VavgB + cutoffB * sigmaVB
       endif
    ...
    if (master) then
      ...
      write(mdout,'(a,4I10)')   '| igamd,iEP,iED,iEB   = ', igamd,iEP,iED,iEB
      endif
      ...
      write(mdout,'(a,3f14.4)') '| sigma0P,sigma0D,sigma0B  = ',sigma0P,sigma0D,sigma0B
      endif
      ...
      write(mdout,'(a,4f14.4)') '| Initial dihedral energy statistics: VmaxD,VminD,VavgD,sigmaVD = ', &
                                     VmaxD,VminD,VavgD,sigmaVD
      if(igamd.ge.20.and.igamd.le.28)then
      write(mdout,'(a,4f14.4)') '| Initial bonded energy statistics: VmaxB,VminB,VavgB,sigmaVB = ', &
                                    VmaxB,VminB,VavgB,sigmaVB
      endif
      ...
      if(igamd.ge.20.and.igamd.le.28)then
      write(mdout,'(a,3f14.4)') '| GaMD third potential boost: EthreshB,k_B,k0B = ',EthreshB,k_B,k0B
      endif
    endif
  endif
...
!the only valid ips option in pmemd is ips=1
  ...
  if ( ips .ne. 0 .and. ((lj1264 .gt. 0) .or. (plj1264 .gt. 0))) then ! New2022
...
! Thermodynamic Integration validation
  if (icfe .eq. 0) then
    ...
    if (ntp .ne. 0 .and. ntp .ne. 1  .and. ntp .ne. 4) then
      if(.not.(igamd.eq.12.or.igamd.eq.13.or.(igamd.ge.110 .and. igamd.le.120).or.&
              ...
              (igamd.eq.18 .or. igamd.eq.19).or.(igamd.eq.20 .or.igamd.eq.21).or.&
              igamd.eq.22.or.igamd.eq.26.or.igamd.eq.24.or.igamd.eq.27.or.igamd.eq.28))then
  ...
  if (ifsc .ne. 0) then
    ...
    if (ntf .gt. 1) then
      if(.not.(igamd.eq.12.or.igamd.eq.13.or.(igamd.ge.110 .and. igamd.le.120).or.&
            (igamd.eq.14 .or. igamd.eq.15).or.(igamd.eq.16 .or. igamd.eq.17).or.&
            (igamd.eq.18 .or. igamd.eq.19).or.(igamd.ge.20.and.igamd.le.28)))then
  ...
  if (ifreaf .ne. 0) then
    if (reaf_tau<0) then
      write(mdout, '(a,a)') error_hdr, 'READ tau must be set positive'
    end if 
    gti_cpu_output=0
    write(mdout, '(a,a)') '| gti_cpu_output is enforced to be 0 when READ is on.'
  end if
...
! Reservoir REMD options
  ...
  if (icfe.le.0) then    
    ...
    if (rremd_type .ne. 0 .and. rremd_type .lt. 4 .or. rremd_type .gt. 6)  &
        write(mdout, '(a,a)') error_hdr, &
          'The only rremd_type values allowed for TI 4, 5, or 6'
  end if

  if (rremd_type .eq. 1 .or. rremd_type .eq. 2) then
    if (remd_method .ne. 1 .and. remd_method .ne. 3) then
      write(mdout, '(a,a)') error_hdr, &
        'The only -rem value currently supported for reservoir remd is 1 or 3!'
      inerr = 1
    end if
...
!! End Hybrid Solvent REMD options
  ...
  if (remd_method .gt. 0) then
    ...
    if (gremd_acyc .lt.0 .and. icfe.gt.0) gremd_acyc =1

    if (mod(numgroups, 2) .ne. 0 .and. gremd_acyc .le. 0) then
...
! If EMIL was requested, make sure it was compiled in.
  if( emil_do_calc .gt. 0 ) then
    ...
    write (mdout, '(a)') '      Look at the cmake documentation and "cmakelists" files for more info.'
    inerr = 1
#endif
  end if
...
! ASM
#if defined(MPI)
   if (asm /= 0 .and. asm /= 1) then
      write(mdout,'(a)') 'asm must be either 0 or 1'
      inerr = 1
   end if
#endif
...
subroutine post_prm_validate_mdin_ctrl_dat
  ...
  if ((lj1264 .eq. 1) .or. (plj1264 .eq. 1)) then ! New2022
#if !defined(GTI)
#ifdef CUDA
...
subroutine print_mdin_ctrl_dat(remd_method, cph_igb, ce_igb, cpein_specified, rremd_type)
...
#ifdef MPI
  if (remd_method .ne. 0) then
    write(mdout,'(/a)') 'Replica exchange'
    write(mdout,'(5x,4(a,i8))') 'numexchg=',numexchg,', rem=',remd_method
    if(rremd_type .gt. 0) then
       write(mdout,'(/a)') 'Parameters for Reservoir REMD'
       write(mdout,'(5x,(a,i8))') 'rremd_type = ', rremd_type ! Koushik
       write(mdout,'(5x,(a,i8))') 'reservoir_exchange_step = ', reservoir_exchange_step
    end if
  end if
#endif
  ...
  if (ionstepvelocities > 0) &
     write(mdout, '(5x, a,i8)') 'ionstepvelocities    =',ionstepvelocities
  ...
  if (igb .eq. 0) then
    ...
    if (lj1264 .ne. 0) then ! New2022
      write(mdout, '(5x,a,i8)') 'lj1264  =', lj1264
    end if
    if (plj1264 .ne. 0) then
      write(mdout, '(5x,a,i8)') 'plj1264  =', plj1264
    end if
  else
      write(mdout,'(5x,3(a,f10.5))') 'dielc   =', dielc, &
  ...
  if (gbion == 1) then
     write(mdout,'(/a)') 'GB with ion atoms treated separately:'
     write(mdout,'(5x,a,i8)') 'gbion  =', gbion
     write(mdout,'(5x,a,f15.12)') 'gi_coef  =', gi_coef
  end if

  if (gbion == 2) then
     write(mdout,'(/a)') 'GB with ion atoms treated separately:'
     write(mdout,'(5x,a,i8)') 'gbion  =', gbion
     write(mdout,'(5x,a,f15.12)') 'gi_coef_1  =', gi_coef_1
     write(mdout,'(5x,a,f15.12)') 'gi_coef_2  =', gi_coef_2
     write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_1  =', gb_neckscale_ion_1
     write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_2  =', gb_neckscale_ion_2
     write(mdout,'(5x,a,f15.12)') 'intdiel_ion_1  =', intdiel_ion_1
     write(mdout,'(5x,a,f15.12)') 'intdiel_ion_2  =', intdiel_ion_2
  end if

  if (gbion == 3) then
    write(mdout,'(/a)') 'GB with cation & anion atoms treated separately:'
    write(mdout,'(5x,a,i8)') 'gbion  =', gbion
    write(mdout,'(5x,a,f15.12)') 'gi_coef_1_p  =', gi_coef_1_p
    write(mdout,'(5x,a,f15.12)') 'gi_coef_1_n  =', gi_coef_1_n
    write(mdout,'(5x,a,f15.12)') 'gi_coef_2_pp  =', gi_coef_2_pp
    write(mdout,'(5x,a,f15.12)') 'gi_coef_2_pn  =', gi_coef_2_pn
    write(mdout,'(5x,a,f15.12)') 'gi_coef_2_nn  =', gi_coef_2_nn
    write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_1_p  =', gb_neckscale_ion_1_p
    write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_1_n  =', gb_neckscale_ion_1_n
    write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_2_pp  =', gb_neckscale_ion_2_pp
    write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_2_pn  =', gb_neckscale_ion_2_pn
    write(mdout,'(5x,a,f15.12)') 'gb_neckscale_ion_2_nn  =', gb_neckscale_ion_2_nn
    write(mdout,'(5x,a,f15.12)') 'intdiel_ion_1_p  =', intdiel_ion_1_p
    write(mdout,'(5x,a,f15.12)') 'intdiel_ion_1_n  =', intdiel_ion_1_n
    write(mdout,'(5x,a,f15.12)') 'intdiel_ion_2_pp  =', intdiel_ion_2_pp
    write(mdout,'(5x,a,f15.12)') 'intdiel_ion_2_pn  =', intdiel_ion_2_pn
    write(mdout,'(5x,a,f15.12)') 'intdiel_ion_2_nn  =', intdiel_ion_2_nn
  end if
  ...
  if( ntr == 1 ) write(mdout,'(5x,a,f10.5)') 'restraint_wt =', restraint_wt
...
#ifdef GTI
    if (scbeta .lt. 0.0) then
      if (gti_auto_alpha .gt. 0) scbeta=4.0
    endif

    if (gti_lam_sch .gt.0 ) then
      if (gti_ele_sc .eq. -99) gti_ele_sc=1
      if (gti_vdw_sc .eq. -99) gti_vdw_sc=1
      if (scalpha .lt. 0.0) scalpha=0.2
      if (scbeta .lt. 0.0) scbeta=50
    else
      if (gti_ele_sc .eq. -99) gti_ele_sc=0
      if (gti_vdw_sc .eq. -99) gti_vdw_sc=0
      if (scalpha .lt. 0.0) scalpha=0.5
      if (scbeta .lt. 0.0)  scbeta=12.0
    end if
#endif
...
! Targeted MD not currently supported...
!
! if (itgtmd .ne. 0) then
...
! External library information
  if (iextpot .ne. 0) then
    ...
    write(mdout, '(5x,a,x,a)') 'extprog =', trim(extprog)
    if (json .ne. '') write(mdout, '(5x,a,x,a)') 'json =', trim(json)
  end if
