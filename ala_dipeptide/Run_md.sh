...
pmemd.cuda -O -i $PWD/min2.in -p $PWD/complex.prmtop -c $PWD/min1.restrt -o $PWD/min2.out -r $PWD/min2.restrt
...
&cntrl
   ...
   temp0=355, gamma_ln=1.0, ig=-1,
   ntp=0, ntc=2, ntf=2,
   ntb=1, nmropt=1
 /
 &wt
   TYPE='TEMP0', ISTEP1=1, ISTEP2=500000,
   VALUE1=10.0, VALUE2=355.0,
 /
 &wt TYPE='END' /
EOF
...
&cntrl
   ...
   pseudoBD_cycles=250,
   refer_1_start=1, refer_1_end=8,
   refer_2_start=15, refer_2_end=22,
   solvent_start=23, solvent_end=10369
/
EOF

/tscc/lustre/ddn/scratch/h9wei/work_4/amber24/bin/pmemd.cuda_SPFP -O -i $PWD/md.in -p $PWD/complex.prmtop -c $PWD/heat2.rst -o $PWD/md.out -r $PWD/md.rst -x mdcrd
