...
! Haixin tmp
  double precision      :: tmp_vel(3, atm_cnt)
  double precision      :: distance_record(atm_cnt)
  double precision      :: tmp_crd(3, atm_cnt), tmp_frc(3, atm_cnt)
  double precision      :: relative_dis(3), random_direction(3,6)
  double precision      :: moment(3,6), moment_last(3,6), refer_mass(6)
  double precision      :: tmp_xmin, tmp_xmax, tmp_ymin, tmp_ymax, tmp_zmin, tmp_zmax
  integer               :: tmp_mark(atm_cnt), tmp_record(atm_cnt), tmp_count(6)
...
! Haixin tmp routine
#ifdef CUDA
if (pseudoBD_cycles > 0) then
  if (total_nstep .eq. 0) then
    moment = 0.0
    moment_last = 0.0
    tmp_count = 0
    refer_mass = 0.0
    if (refer_1_start > 0) refer_mass(1) = sum(mass(refer_1_start:refer_1_end))
    if (refer_2_start > 0) refer_mass(2) = sum(mass(refer_2_start:refer_2_end))
    if (refer_3_start > 0) refer_mass(3) = sum(mass(refer_3_start:refer_3_end))
    if (refer_4_start > 0) refer_mass(4) = sum(mass(refer_4_start:refer_4_end))
    if (refer_5_start > 0) refer_mass(5) = sum(mass(refer_5_start:refer_5_end))
    if (refer_6_start > 0) refer_mass(6) = sum(mass(refer_6_start:refer_6_end))
  end if
  if (mod((total_nstep+1), pseudoBD_cycles) .eq.  0) then
    call gpu_download_vel(tmp_vel)
    if (refer_1_start > 0) then
      moment(:,1) = 0.0
      do j = refer_1_start, refer_1_end
        moment(:,1) = moment(:,1) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_2_start > 0) then
      moment(:,2) = 0.0
      do j = refer_2_start, refer_2_end
        moment(:,2) = moment(:,2) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_3_start > 0) then
      moment(:,3) = 0.0
      do j = refer_3_start, refer_3_end
        moment(:,3) = moment(:,3) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_4_start > 0) then
      moment(:,4) = 0.0
      do j = refer_4_start, refer_4_end
        moment(:,4) = moment(:,4) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_5_start > 0) then
      moment(:,5) = 0.0
      do j = refer_5_start, refer_5_end
        moment(:,5) = moment(:,5) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_6_start > 0) then
      moment(:,6) = 0.0
      do j = refer_6_start, refer_6_end
        moment(:,6) = moment(:,6) + mass(j) * tmp_vel(:,j)
      end do
    end if
    if (refer_1_start > 0) then
      moment(:,1) = moment(:,1) - refer_mass(1)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,1),moment_last(:,1)) < 0.0 .and. tmp_count(1) < 2) &
      tmp_count(1) = tmp_count(1) + 1
    else
      tmp_count(1) = 2
    end if
    if (refer_2_start > 0) then
      moment(:,2) = moment(:,2) - refer_mass(2)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,2),moment_last(:,2)) < 0.0 .and. tmp_count(2) < 2) &
      tmp_count(2) = tmp_count(2) + 1
    else
      tmp_count(2) = 2
    end if
    if (refer_3_start > 0) then
      moment(:,3) = moment(:,3) - refer_mass(3)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,3),moment_last(:,3)) < 0.0  .and. tmp_count(3) < 2) &
      tmp_count(3) = tmp_count(3) + 1
    else
      tmp_count(3) = 2
    end if
    if (refer_4_start > 0) then
      moment(:,4) = moment(:,4) - refer_mass(4)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,4),moment_last(:,4)) < 0.0 .and. tmp_count(4) < 2) &
      tmp_count(4) = tmp_count(4) + 1
    else
      tmp_count(4) = 2
    end if
    if (refer_5_start > 0) then
      moment(:,5) = moment(:,5) - refer_mass(5)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,5),moment_last(:,5)) < 0.0 .and. tmp_count(5) < 2) &
      tmp_count(5) = tmp_count(5) + 1
    else
      tmp_count(5) = 2
    end if
    if (refer_6_start > 0) then
      moment(:,6) = moment(:,6) - refer_mass(6)*sum(moment,Dim=2)/sum(refer_mass)
      if (dot_product(moment(:,6),moment_last(:,6)) < 0.0 .and. tmp_count(6) < 2) &
      tmp_count(6) = tmp_count(6) + 1
    else
      tmp_count(6) = 2
    end if
  end if
...
if (sum(tmp_count) .eq. 12) then

  moment = 0.0
  moment_last = 0.0
  tmp_count = 0
  !write(mdout,*) 'Haixin change vel: ', total_nstep + 1

  call gpu_vrand_reset_velocities(temp0 * factt, half_dtx)
  call gpu_download_vel(tmp_vel)
  call gpu_download_frc(tmp_frc)
  do j = 1, atm_cnt
    wfac = atm_mass_inv(j) * half_dtx
    tmp_vel(:, j) = tmp_vel(:, j) + tmp_frc(:, j) * wfac
  end do

  call gpu_download_crd(tmp_crd)
  call wrap_molecules(tmp_crd)

  distance_record = 25.0
  tmp_record = 0

  if (refer_1_start > 0) then
    tmp_xmin = tmp_crd(1,refer_1_start); tmp_xmax = tmp_crd(1,refer_1_start)
    tmp_ymin = tmp_crd(2,refer_1_start); tmp_ymax = tmp_crd(2,refer_1_start)
    tmp_zmin = tmp_crd(3,refer_1_start); tmp_zmax = tmp_crd(3,refer_1_start)
    do j = refer_1_start, refer_1_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
      if (tmp_crd(1,j) > tmp_xmax) tmp_xmax = tmp_crd(1,j)
      if (tmp_crd(2,j) > tmp_ymax) tmp_ymax = tmp_crd(2,j)
      if (tmp_crd(3,j) > tmp_zmax) tmp_zmax = tmp_crd(3,j)
    end do

    tmp_xmin = tmp_xmin - 5.0
    tmp_ymin = tmp_ymin - 5.0
    tmp_zmin = tmp_zmin - 5.0
    tmp_xmax = tmp_xmax + 5.0
    tmp_ymax = tmp_ymax + 5.0
    tmp_zmax = tmp_zmax + 5.0

    tmp_mark = 0
    do j = solvent_start, solvent_end
      if ((tmp_crd(1,j) > tmp_xmin .and. tmp_crd(1,j) < tmp_xmax) .or. &
          (tmp_crd(1,j) > tmp_xmin+pbc_box(1) .and. tmp_crd(1,j) < tmp_xmax+pbc_box(1)) .or. &
          (tmp_crd(1,j) > tmp_xmin-pbc_box(1) .and. tmp_crd(1,j) < tmp_xmax-pbc_box(1))) then
      if ((tmp_crd(2,j) > tmp_ymin .and. tmp_crd(2,j) < tmp_ymax) .or. &
          (tmp_crd(2,j) > tmp_ymin+pbc_box(2) .and. tmp_crd(2,j) < tmp_ymax+pbc_box(2)) .or. &
          (tmp_crd(2,j) > tmp_ymin-pbc_box(2) .and. tmp_crd(2,j) < tmp_ymax-pbc_box(2))) then
      if ((tmp_crd(3,j) > tmp_zmin .and. tmp_crd(3,j) < tmp_zmax) .or. &
          (tmp_crd(3,j) > tmp_zmin+pbc_box(3) .and. tmp_crd(3,j) < tmp_zmax+pbc_box(3)) .or. &
          (tmp_crd(3,j) > tmp_zmin-pbc_box(3) .and. tmp_crd(3,j) < tmp_zmax-pbc_box(3))) then
    ...
    do j = refer_1_start, refer_1_end
    do i = solvent_start, solvent_end
    if (tmp_mark(i) .eq. 1) then
      relative_dis(1) = min(abs(tmp_crd(1,j)-tmp_crd(1,i)),pbc_box(1)-abs(tmp_crd(1,j)-tmp_crd(1,i)))
      relative_dis(2) = min(abs(tmp_crd(2,j)-tmp_crd(2,i)),pbc_box(2)-abs(tmp_crd(2,j)-tmp_crd(2,i)))
      relative_dis(3) = min(abs(tmp_crd(3,j)-tmp_crd(3,i)),pbc_box(3)-abs(tmp_crd(3,j)-tmp_crd(3,i)))
      scaltp = dot_product(relative_dis,relative_dis)
      if (scaltp < 9.0) tmp_record(j) = 1 ! macromolecule surface, not refer_1
      if (scaltp < distance_record(i)) then
        tmp_record(i) = 1 ! refer_1
        distance_record(i) = scaltp
      end if
    end if
    end do
    end do
  ...
  if (refer_2_start > 0) then
    tmp_xmin = tmp_crd(1,refer_2_start); tmp_xmax = tmp_crd(1,refer_2_start)
    tmp_ymin = tmp_crd(2,refer_2_start); tmp_ymax = tmp_crd(2,refer_2_start)
    tmp_zmin = tmp_crd(3,refer_2_start); tmp_zmax = tmp_crd(3,refer_2_start)
    do j = refer_2_start, refer_2_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
  ...
  if (refer_3_start > 0) then
    tmp_xmin = tmp_crd(1,refer_3_start); tmp_xmax = tmp_crd(1,refer_3_start)
    tmp_ymin = tmp_crd(2,refer_3_start); tmp_ymax = tmp_crd(2,refer_3_start)
    tmp_zmin = tmp_crd(3,refer_3_start); tmp_zmax = tmp_crd(3,refer_3_start)
    do j = refer_3_start, refer_3_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
  ...
  if (refer_4_start > 0) then
    tmp_xmin = tmp_crd(1,refer_4_start); tmp_xmax = tmp_crd(1,refer_4_start)
    tmp_ymin = tmp_crd(2,refer_4_start); tmp_ymax = tmp_crd(2,refer_4_start)
    tmp_zmin = tmp_crd(3,refer_4_start); tmp_zmax = tmp_crd(3,refer_4_start)
    do j = refer_4_start, refer_4_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
  ...
  if (refer_5_start > 0) then
    tmp_xmin = tmp_crd(1,refer_5_start); tmp_xmax = tmp_crd(1,refer_5_start)
    tmp_ymin = tmp_crd(2,refer_5_start); tmp_ymax = tmp_crd(2,refer_5_start)
    tmp_zmin = tmp_crd(3,refer_5_start); tmp_zmax = tmp_crd(3,refer_5_start)
    do j = refer_5_start, refer_5_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
  ...
  if (refer_6_start > 0) then
    tmp_xmin = tmp_crd(1,refer_6_start); tmp_xmax = tmp_crd(1,refer_6_start)
    tmp_ymin = tmp_crd(2,refer_6_start); tmp_ymax = tmp_crd(2,refer_6_start)
    tmp_zmin = tmp_crd(3,refer_6_start); tmp_zmax = tmp_crd(3,refer_6_start)
    do j = refer_6_start, refer_6_end
      if (tmp_crd(1,j) < tmp_xmin) tmp_xmin = tmp_crd(1,j)
      if (tmp_crd(2,j) < tmp_ymin) tmp_ymin = tmp_crd(2,j)
      if (tmp_crd(3,j) < tmp_zmin) tmp_zmin = tmp_crd(3,j)
  ...
  !tmp_mark(1:6) = 0
  call vrand_set_velocities(6, random_direction, atm_mass_inv, temp0 * factt)
  !if (refer_1_start > 0) then
  !  moment(:,1) = 0.0
  !  do j = refer_1_start, refer_1_end
  !    moment(:,1) = moment(:,1) + mass(j) * tmp_vel(:,j)
  ...
  !  end do
  !end if
  !random_direction = moment

  !if (refer_2_equal .ne. 0) random_direction(:,refer_2_equal) = random_direction(:,refer_2_equal) + moment(:,2)
  !if (refer_3_equal .ne. 0) random_direction(:,refer_3_equal) = random_direction(:,refer_3_equal) + moment(:,3)
  !if (refer_4_equal .ne. 0) random_direction(:,refer_4_equal) = random_direction(:,refer_4_equal) + moment(:,4)
  !if (refer_5_equal .ne. 0) random_direction(:,refer_5_equal) = random_direction(:,refer_5_equal) + moment(:,5)
  !if (refer_6_equal .ne. 0) random_direction(:,refer_6_equal) = random_direction(:,refer_6_equal) + moment(:,6)

  if (refer_2_equal .ne. 0) random_direction(:,2) = random_direction(:,refer_2_equal)
  if (refer_3_equal .ne. 0) random_direction(:,3) = random_direction(:,refer_3_equal)
  if (refer_4_equal .ne. 0) random_direction(:,4) = random_direction(:,refer_4_equal)
  if (refer_5_equal .ne. 0) random_direction(:,5) = random_direction(:,refer_5_equal)
  if (refer_6_equal .ne. 0) random_direction(:,6) = random_direction(:,refer_6_equal)

  do i = solvent_start, solvent_end
  if (tmp_record(i) .eq. 1) then ! refer_1
    relative_dis(:) = random_direction(:,1) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(1) = tmp_mark(1) + 1
  end if
  if (tmp_record(i) .eq. 2) then ! refer_2
    relative_dis(:) = random_direction(:,2) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(2) = tmp_mark(2) + 1
  end if
  if (tmp_record(i) .eq. 3) then ! refer_3
    relative_dis(:) = random_direction(:,3) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(3) = tmp_mark(3) + 1
  end if
  if (tmp_record(i) .eq. 4) then ! refer_4
    relative_dis(:) = random_direction(:,4) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(4) = tmp_mark(4) + 1
  end if
  if (tmp_record(i) .eq. 5) then ! refer_5
    relative_dis(:) = random_direction(:,5) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(5) = tmp_mark(5) + 1
  end if
  if (tmp_record(i) .eq. 6) then ! refer_6
    relative_dis(:) = random_direction(:,6) ! this is a random direction
    tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
    tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
    tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
    !tmp_mark(6) = tmp_mark(6) + 1
  end if
  end do

  if (refer_1_start > 0) then
    relative_dis(:) = random_direction(:,1) ! this is a random direction
    do i = refer_1_start, refer_1_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
      end if
  ...
  if (refer_2_start > 0) then
    relative_dis(:) = random_direction(:,2) ! this is a random direction
    do i = refer_2_start, refer_2_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
  ...
  if (refer_3_start > 0) then
    relative_dis(:) = random_direction(:,3) ! this is a random direction
    do i = refer_3_start, refer_3_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
  ...
  if (refer_4_start > 0) then
    relative_dis(:) = random_direction(:,4) ! this is a random direction
    do i = refer_4_start, refer_4_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
  ...
  if (refer_5_start > 0) then
    relative_dis(:) = random_direction(:,5) ! this is a random direction
    do i = refer_5_start, refer_5_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
  ...
  if (refer_6_start > 0) then
    relative_dis(:) = random_direction(:,6) ! this is a random direction
    do i = refer_6_start, refer_6_end
      if (tmp_record(i) .eq. 1) then
        tmp_vel(1,i) = sign(tmp_vel(1,i),relative_dis(1))
        tmp_vel(2,i) = sign(tmp_vel(2,i),relative_dis(2))
        tmp_vel(3,i) = sign(tmp_vel(3,i),relative_dis(3))
  ...
  !write(mdout,*) 'solvent number: ', tmp_mark(1:6)

  do j = 1, atm_cnt
    wfac = atm_mass_inv(j) * half_dtx
    tmp_vel(:, j) = tmp_vel(:, j) - tmp_frc(:, j) * wfac
  end do
  call gpu_upload_vel(tmp_vel)
